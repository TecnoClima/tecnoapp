...
export default function PlanFilters(props) {
  ...
  function completeDevices(event) {
    ...
    if (!input) {
      ...
      data.map(
        (date) =>
          !devices.map((e) => e.code).includes(date.code) &&
          devices.push({ code: date.code, name: date.device })
      );
      setDeviceList(
        devices.filter(
          (device) => device.code.includes(input) || device.name.includes(input)
        )
      );
    }
  }
  ...
  useEffect(() => {
    let workers = [];
    if (filters.strategy) {
      workers = programList.find(
        (strategy) => strategy.name === filters.strategy
      ).people;
    } else {
      programList.map((program) =>
        program.people.map(
          (worker) =>
            !workers.map((element) => element.id).includes(worker.id) &&
            workers.push(worker)
        )
      );
    }
  ...
  useEffect(() => {
    ...
    programList.map(
      (strategy) =>
        !supervisors.map((e) => e.id).includes(strategy.supervisor.id) &&
        supervisors.push(strategy.supervisor)
    );
  ...
  useEffect(() => {
    ...
    if (userData.plant) {
      (!deviceFullList || !deviceFullList[0]) &&
        dispatch(getDeviceList(userData.plant));
  ...
  useEffect(
    () =>
      ...
      setDeviceList(
        partialList
          ? partialList.sort((a, b) => (a.name > b.name ? 1 : -1))
          : []
      ),
    [partialList, filters.line]
  );
  ...
  function handleComplete(type, event) {
    ...
    if (type === "value") {
      ...
      value
        ? (newFilter[`${type}Complete`] = value)
        : delete newFilter[`${type}Complete`];
    }
    setFilters(newFilter);
    select && select(newFilter);
  }
  ...
  function PlanItemFilter(props) {
    ...
    return (
      <select
        title={item}
        className={`form-select w-auto p-0 pe-5 ${className}`}
            ...
            <div class="input-group w-auto">
              ...
              <PlanItemFilter
                ...
                extraAction={(value) => {
                  ...
                  //   setLineList(
                  //     value && (filters.plant || userData.plant)
                ...
                <div>
                  ...
                  {deviceList[0] && (
                    <ul className="autoComplete">
                      {deviceList[1] && (
                        <li onClick={() => deviceSelect("all")}>
                          Toda la lista
                        </li>
                      )}
                      {deviceList.map((device, index) => (
                        <li
                          onClick={() => deviceSelect(device.code)}
                          key={index}
                        >{`[${device.code}] ${device.name}`}</li>
                      ))}
                    </ul>
                  )}
                </div>
