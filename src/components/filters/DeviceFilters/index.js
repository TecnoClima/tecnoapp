...
function applyFilters(device, filters) {
  ...
  if (rangeFilters)
    for (let key of Object.keys(rangeFilters)) {
      if (
        device[key] < rangeFilters[key].min ||
        device[key] > rangeFilters[key].max
      )
        check = false;
    }
  if (includeFilters)
    for (let key of Object.keys(includeFilters)) {
      if (typeof includeFilters[key] === "string") {
        if (
          !device[key].toLowerCase().includes(includeFilters[key].toLowerCase())
        )
          check = false;
      } else {
        for (let element of device[key]) {
          if (element.toLowerCase.includes(includeFilters[key].toLowerCase()))
            check = false;
        }
      }
    }
...
function RangeInput(props) {
  ...
  const [unit, setUnit] = useState(
    equivalency ? Object.keys(equivalency)[0] : undefined
  );
  ...
  function selectValues(values, unit) {
    ...
    if (!keys[0]) return;
    for (let key of keys)
      newValues[key] = newValues[key] * (unit ? equivalency[unit] : 1);
...
export default function DeviceFilters(props) {
  ...
  useEffect(
    () =>
      list &&
      setFilteredList(list.filter((device) => applyFilters(device, filters))),
    [list, filters]
  );
        ...
        <div className="row">
          <div className="col">
            <select
              ...
              defaultValue={
                filters.valueFilters ? filters.valueFilters[item] : undefined
              }
              ...
              <div className="col">
                <form className="d-flex">
                  ...
                  <button
                    ...
                    onClick={(event) => includeFilter("name", name, event)}
                  >
                    BUSCAR
                  </button>
                </form>
